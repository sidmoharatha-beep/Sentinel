...
function minutesAgo(iso: string) { return Math.round((Date.now()-parseUTC(iso).getTime())/60000); }
...
export default function LiveMap() {
  ...
  // Init map synchronously — no dynamic import race
  useEffect(() => {
    if (mapRef.current || !mapElRef.current) return;
    const map = L.map(mapElRef.current, { center:[PLANT_CENTER.lat,PLANT_CENTER.lng], zoom:17 });
    ...
    CHECKPOINTS.forEach((cp, i) => {
      ...
      L.marker([cp.lat,cp.lng],{icon}).addTo(map).bindPopup(`<b>${cp.name}</b><br/><small>${cp.code} · ${cp.type}</small>`);
    });
  ...
  // Update guard markers — depends on mapReady too, so it fires as soon as
  // the map finishes initializing even if data already arrived earlier.
  useEffect(() => {
    ...
    Object.values(latestByGuard).forEach((g:any,idx)=>{
      ...
      if (markersRef.current[g.employee_id]) {
        ...
        markersRef.current[g.employee_id] = L.marker([g.latitude,g.longitude],{icon})
  ...
  }, [timeline, selectedGuard, mapReady]);
      ...
      <div className="flex flex-col -mt-4 -mx-4" style={{height:'calc(100vh - 64px)'}}>
        <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-border shrink-0 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <h2 className="text-base font-bold text-primary flex items-center gap-2"><MapPin size={15} className="text-accent"/> Live Map — ITC ICML, Khordha</h2>
            ...
            <span className="text-xs text-text-muted hidden sm:block">Updated: {lastRefresh.toLocaleTimeString('en-IN',{hour12:false})}</span>
          </div>
          <div className="flex items-center gap-3 text-xs text-text-muted">
            ...
            <button onClick={loadData} className="flex items-center gap-1 px-2 py-1 rounded-lg border border-border hover:bg-surface-alt"><RefreshCw size={11} className={loading?'animate-spin':''}/> Refresh</button>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-64 border-l border-border bg-white flex flex-col overflow-hidden shrink-0">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {guards.length===0 ? <p className="px-3 py-6 text-center text-xs text-text-muted">No guard activity today</p> : guards.map((g:any,idx)=>{
                const mins = minutesAgo(g.scanned_at);
                const color = GUARD_COLORS[idx%GUARD_COLORS.length];
                const sel = selectedGuard===g.employee_id;
                return (
                  <button key={g.employee_id} onClick={()=>{
                    ...
                    if (g.latitude&&g.longitude&&mapRef.current) { mapRef.current.flyTo([g.latitude,g.longitude],18,{duration:1}); markersRef.current[g.employee_id]?.openPopup(); }
                  }} className={cn('w-full text-left px-3 py-2 border-b border-border hover:bg-surface-alt', sel&&'bg-accent/5 border-l-2 border-l-accent')}>
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0" style={{background:color}}>{g.guard_name?.charAt(0)||'G'}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center">
                          <p className="text-xs font-semibold text-primary truncate">{g.guard_name}</p>
                          <span className={cn('text-[10px] font-medium px-1 py-0.5 rounded-full', mins>90?'bg-red-50 text-red-600':'bg-green-50 text-green-600')}>{mins}m</span>
                        </div>
            ...
            <div className="border-t border-border">
              ...
              <div className="overflow-y-auto max-h-40 pb-2">
                {CHECKPOINTS.map((cp,i)=>{
                  ...
                  return (
                    <div key={cp.id} onClick={()=>mapRef.current?.flyTo([cp.lat,cp.lng],19,{duration:1})} className="flex items-center gap-2 px-3 py-1 hover:bg-surface-alt cursor-pointer">
                      <div className={cn('w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold text-white shrink-0', done?'bg-green-500':cp.type==='critical'?'bg-red-400':'bg-gray-300')}>{done?'✓':i+1}</div>
