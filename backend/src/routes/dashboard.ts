...
app.get('/insights', requireRole('system_admin', 'security_manager', 'security_supervisor'), async (c) => {
  ...
  // Each check runs independently — if one query fails (e.g. no data yet,
  // schema drift), we log it and continue rather than 500ing the whole
  // endpoint and losing every other insight.
  try {
  ...
  } catch (e) { console.error('insights: compliance_trend failed', e); }
  ...
  } catch (e) { console.error('insights: top_category failed', e); }
  ...
  } catch (e) { console.error('insights: risk_area failed', e); }
  ...
  } catch (e) { console.error('insights: weak_shift failed', e); }
  ...
  } catch (e) { console.error('insights: stale_critical failed', e); }
  ...
  } catch (e) { console.error('insights: active_reporter failed', e); }
